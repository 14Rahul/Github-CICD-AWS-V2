...
select_eks_cluster() {
    if [ "$ENV" == "dev" ] || [ "$ENV" == "qa" ]; then
        ...
        if [ "$REGION" == "us-east-2" ]; then
            ...
            aws eks update-kubeconfig --name github-cicd --region us-east-2 --role-arn arn:aws:iam::337909738694:role/github-user-role
...
build_and_push_image() {
  ...
  aws ecr get-login-password --region "$REGION" | docker login --username AWS --password-stdin "337909738694.dkr.ecr.$REGION.amazonaws.com"
  aws codeartifact login --tool pip --repository propy --domain harekrishnahareram --domain-owner 337909738694 --region us-east-2
...
if [ "$ENV" == "prod" ]; then # || [ "$ENV" == "pci" ] || [ "$ENV" == "pci-irs" ]; then # Testing apps services in non master or main branch
...
if [ "$FREEZE" == true ] && [ "$BRANCH" == "main" ]; then
    if [ "$ENV" == "prod" ] || [ "$ENV" == "pci-irs" ] || [ "$ENV" == "pci" ]; then
        cd "$REPO_HOME/repo"
        ...
        export FREEZE_CONTAINER="freeze_container_$(date +'%Y%m%d%H%M%S')"
...
# if [ "$ENV" == "prod" ]; then
#      if [ "$SERVICE" == "asr-daemon" ] || [ "$ENV" == "hermes" ] || [ "$ENV" == "callback-receiver" ]; then
#             echo "Logging in to PCI EKS in US Region"
#             aws eks update-kubeconfig --name OhEKSIntApiPci --region us-east-2 --role-arn arn:aws:iam::532968567499:role/admin-role-OhEKSIntApiPci
#             ENV="pci"
#             IMAGE_TAG="pci-latest"
#             build_and_push_image || log_and_exit "Failed to build and push the image. Exiting."
#             deploy || log_and_exit "Failed to deploy. Exiting."
#      elif [ "$SERVICE" == "bifrost" ]; then
#             ENV="pci"
#             IMAGE_TAG="pci-latest"
#             build_and_push_image || log_and_exit "Failed to build and push the image. Exiting."
#      fi
